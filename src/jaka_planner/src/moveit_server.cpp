...
int main(int argc, char *argv[])
{
    ...
    std::array<char, 128> sdk_version{};
    int sdk_version_ret = robot.get_sdk_version(sdk_version.data());
    if (sdk_version_ret == ERR_SUCC)
    {
        RCLCPP_INFO(node->get_logger(), "JAKA SDK version: %s", sdk_version.data());
    }
    else
    {
        const auto error = mapErr.find(sdk_version_ret);
        const char *error_name = error != mapErr.end() ? error->second.c_str() : "UNKNOWN_ERROR";
        RCLCPP_WARN(node->get_logger(), "get_sdk_version failed: %d (%s)",
                    sdk_version_ret, error_name);
    }

    // Connect to robot
    int ret = robot.login_in(robot_ip.c_str(), false);
    const auto login_error = mapErr.find(ret);
    const char *login_error_name = ret == ERR_SUCC
        ? "ERR_SUCC"
        : (login_error != mapErr.end() ? login_error->second.c_str() : "UNKNOWN_ERROR");
    RCLCPP_INFO(node->get_logger(), "login_in: %d (%s)", ret, login_error_name);
    if (ret !=0) {
        RCLCPP_INFO(node->get_logger(), "login_in failed");
        rclcpp::shutdown();
        return 1;
    }
